  ...
  <div class="checkin-container">
    ...
    <el-card class="today-courses-card">
      <template #header>
        <div class="card-header">
          <h3>今日课程</h3>
          <el-select v-model="selectedCourseId" placeholder="请选择课程" style="width: 300px;" @change="handleCourseChange">
            <el-option v-for="course in todayCourses" :key="course.id" :label="`${course.name} (${formatTime(course.startTime)}-${formatTime(course.endTime)})`" :value="course.id" />
          </el-select>
        </div>
      </template>
    ...
    <el-card class="bookings-card" v-if="selectedCourse">
      <template #header>
        <div class="bookings-header">
          ...
          <div class="header-actions">
            <el-button type="primary" @click="handleBatchCheckIn" :disabled="waitingCheckinCount === 0">
              <el-icon>
                <Check />
              </el-icon>
      ...
      <div class="bookings-grid">
        <div v-for="booking in bookings" :key="booking.id" class="booking-card" :class="{
          ...
          <div class="booking-actions">
            <el-button type="primary" size="small" @click="handleCheckIn(booking)" v-if="booking.status === 'BOOKED'">
              <el-icon>
                <Check />
              </el-icon>
              签到
            ...
            <el-button type="warning" size="small" @click="handleCheckOut(booking)" v-if="booking.status === 'CHECKED_IN'">
              <el-icon>
                <SwitchButton />
              </el-icon>
              签退
            ...
            <el-button type="danger" size="small" @click="handleMarkNoShow(booking)" v-if="booking.status === 'BOOKED'">
              <el-icon>
                <Close />
              </el-icon>
              缺席
    ...
    <!-- 生成签到码对话框 -->
    <el-dialog v-model="codeDialog.visible" title="生成签到码" width="400px">
      ...
      <div v-else class="no-code-content">
        <el-select v-model="codeDialog.memberId" placeholder="请选择会员" style="width: 100%;" @change="generateCode">
          <el-option v-for="member in activeMembers" :key="member.id" :label="`${member.realName} (${member.memberNo})`" :value="member.id" />
        </el-select>
      </div>
...
const checkedInCount = computed(() => {
  return bookings.value.filter((b) => b.status === 'CHECKED_IN' || b.status === 'CHECKED_OUT')
    .length
})
...
const handleBatchCheckIn = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确认为 ${waitingCheckinCount.value} 名会员批量签到吗？`,
      '批量签到',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
