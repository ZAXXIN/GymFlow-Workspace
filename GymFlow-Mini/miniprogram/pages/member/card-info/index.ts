 ...
 */
  onCardTap(e: any) {
    ...
    if (card && card.orderItemId) {
      wx.navigateTo({
        url: `/pages/member/card-detail/index?orderItemId=${card.orderItemId}&card=${encodeURIComponent(JSON.stringify(card))}`
      })
    } else {
      console.error('卡片缺少 orderItemId', card)
    }
  }
})